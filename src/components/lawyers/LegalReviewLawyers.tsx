...
type ReviewLawyer = { id: string; slug: string; fullName: string; photoUrl?: string | null; specialization: string; cityId?: string | null; online: true };

export function LegalReviewLawyers({ context, serviceSlug = "semeynye-spory" }: { context: QuestionContext; serviceSlug?: string }) {
  const [lawyers, setLawyers] = useState<ReviewLawyer[]>([]);
  const [message, setMessage] = useState("Проверяем доступных профильных юристов...");

  useEffect(() => {
    let active = true;
    fetch(`/api/legal-review-lawyers/?service=${encodeURIComponent(serviceSlug)}`)
      .then(async (response) => {
  ...
  return (
    <section className="mt-5 rounded-md border border-line bg-white p-4" aria-labelledby="legal-review-lawyers-title">
      <h4 id="legal-review-lawyers-title" className="text-lg font-semibold text-ink">Спросить юриста</h4>
      ...
      {lawyers.length ? <div className="mt-4 grid gap-3 sm:grid-cols-2">{lawyers.map((lawyer) => (
        <article key={lawyer.id} className="grid grid-cols-[3.5rem_1fr] gap-3 rounded-md border border-line p-3">
          ...
          <div className="min-w-0"><p className="break-words font-semibold text-ink">{lawyer.fullName}</p><p className="mt-1 text-xs leading-5 text-zinc-600">{lawyer.specialization}</p><p className="mt-1 text-xs font-semibold text-emerald-700">Онлайн</p><div className="mt-2"><QuestionModal sourcePage={context.route} defaultCityId={lawyer.cityId ?? undefined} defaultServiceSlug={serviceSlug} lawyerId={lawyer.id} context={context} label="Задать вопрос" variant="secondary" /></div></div>
        </article>
      ))}</div> : null}
      <div className="mt-4"><QuestionModal sourcePage={context.route} defaultServiceSlug={serviceSlug} context={context} label="Спросить юриста" variant="primary" /></div>
    </section>
  );
}
