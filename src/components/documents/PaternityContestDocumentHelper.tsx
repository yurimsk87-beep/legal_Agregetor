...
export function PaternityContestDocumentHelper({ scenarioKey }: { scenarioKey: PaternityContestScenarioKey }) {
  ...
  function advance(event: FormEvent) { event.preventDefault(); if (!field || !canAdvance) return; if (safeStep < fields.length - 1) return setStep(safeStep + 1); const result = validatePaternityContest(scenarioKey, values); setDecision(result); setDraft(result.draftText); }
...
function HelperField({ field, value, onChange }: { field: PaternityContestField; value: string; onChange: (name: string, value: string) => void }) {
  const id = `paternity-contest-${field.name}`;
