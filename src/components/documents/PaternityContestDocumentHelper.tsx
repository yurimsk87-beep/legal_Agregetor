...
import { createPaternityContestDocxBlob, getPaternityContestDocxFilename } from "@/lib/paternity-contest-docx";
import { createPaternityContestPdfBlob, getPaternityContestPdfFilename } from "@/lib/paternity-contest-pdf";
import { getVisiblePaternityContestFields, validatePaternityContest } from "@/lib/paternity-contest-validator";
import type { PaternityContestDecision, PaternityContestValues } from "@/lib/paternity-contest-validator";

export function PaternityContestDocumentHelper({ scenarioKey }: { scenarioKey: PaternityContestScenarioKey }) {
  const scenario = PATERNITY_CONTEST_SCENARIOS[scenarioKey];
  const [values, setValues] = useState<PaternityContestValues>({});
  const [step, setStep] = useState(0);
  const [decision, setDecision] = useState<PaternityContestDecision | null>(null);
  const [draft, setDraft] = useState("");
  const [message, setMessage] = useState("");
  const fields = useMemo(() => getVisiblePaternityContestFields(scenarioKey, values), [scenarioKey, values]);
  ...
  const rules = getPaternityContestRules(scenarioKey, decision?.legalPath);

  function setField(name: string, value: string) { setValues((current) => ({ ...current, [name]: value })); setDecision(null); setDraft(""); setMessage(""); }
  function advance(event: FormEvent) { event.preventDefault(); if (!field || !canAdvance) return; if (safeStep < fields.length - 1) return setStep(safeStep + 1); const result = validatePaternityContest(scenarioKey, values); setDecision(result); setDraft(""); }
  function goBack() { if (decision) return setDecision(null); setStep((current) => Math.max(0, current - 1)); }
  async function downloadPdf() { if (!decision) return; try { downloadBlob(await createPaternityContestPdfBlob({ ...decision, draftText: draft }), getPaternityContestPdfFilename(decision.outcomeKey)); setMessage("PDF сформирован. Проверьте предупреждение на первой странице."); } catch { setMessage("PDF не сформирован. Попробуйте ещё раз."); } }
  async function downloadDocx() { if (!draft) return; try { downloadBlob(await createPaternityContestDocxBlob(draft), getPaternityContestDocxFilename(scenario.documentSlug)); setMessage("DOCX сформирован с обязательной маркировкой судебного черновика."); } catch { setMessage("DOCX не сформирован. Используйте PDF или измените ответы."); } }

  return <section id="fill-online" className="scroll-mt-24 border border-line bg-white p-5 shadow-sm sm:p-6">
    <p className="text-sm font-semibold uppercase tracking-wide text-trust">Проверка права на обращение</p><h2 className="mt-2 text-2xl font-semibold text-ink">{scenario.mainDocument}</h2>
    <p className="mt-3 max-w-3xl text-sm leading-6 text-zinc-700">Сервис проверяет заявителя, основание записи, известные при регистрации обстоятельства и специальные ограничения. Он не прогнозирует экспертизу или решение суда.</p>
...
function HelperField({ field, value, onChange }: { field: PaternityContestField; value: string; onChange: (name: string, value: string) => void }) {
  const id = `paternity-contest-${field.name}`; if (isStructuredPartyField(field.name)) return <StructuredPartyField id={id} label={field.label} fieldName={field.name} value={value} onChange={(nextValue) => onChange(field.name, nextValue)} required={field.required} />;
