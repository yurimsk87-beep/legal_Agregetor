...
import { createChildSupportDocxBlob, getChildSupportDocxFilename } from "@/lib/child-support-docx";
import { createChildSupportPdfBlob, getChildSupportPdfFilename } from "@/lib/child-support-pdf";
import { getVisibleChildSupportFields, validateChildSupport } from "@/lib/child-support-validator";
import type { ChildSupportDecision, ChildSupportValues } from "@/lib/child-support-validator";

export function ChildSupportDocumentHelper({ scenarioKey }: { scenarioKey: ChildSupportScenarioKey }) {
  const scenario = CHILD_SUPPORT_SCENARIOS[scenarioKey];
  const [values, setValues] = useState<ChildSupportValues>({});
  const [step, setStep] = useState(0);
  const [decision, setDecision] = useState<ChildSupportDecision | null>(null);
  const [draft, setDraft] = useState("");
  const [message, setMessage] = useState("");
  const fields = useMemo(() => getVisibleChildSupportFields(scenarioKey, values), [scenarioKey, values]);
  ...
  const rules = getChildSupportRules(scenarioKey, decision?.legalPath);
  ...
  function advance(event: FormEvent) {
    ...
    const result = validateChildSupport(scenarioKey, values);
    setDecision(result);
    setDraft("");
  }
  ...
  async function downloadPdf() {
    if (!decision) return;
    try {
      downloadBlob(await createChildSupportPdfBlob({ ...decision, draftText: draft }), getChildSupportPdfFilename(decision.outcomeKey));
  ...
  async function downloadDocx() {
    if (!draft) return;
    try {
      downloadBlob(await createChildSupportDocxBlob(draft), getChildSupportDocxFilename(scenario.documentSlug));
  ...
  return (
    <section id="fill-online" className="scroll-mt-24 border border-line bg-white p-5 shadow-sm sm:p-6">
      ...
      <p className="mt-3 max-w-3xl text-sm leading-6 text-zinc-700">Ответы проверяются детерминированными правилами. Сервис не рассчитывает неподтверждённый доход, задолженность, подсудность или результат суда.</p>
...
function HelperField({ field, value, onChange }: { field: ChildSupportField; value: string; onChange: (name: string, value: string) => void }) {
  const id = `child-support-${field.name}`; if (isStructuredPartyField(field.name)) return <StructuredPartyField id={id} label={field.label} fieldName={field.name} value={value} onChange={(nextValue) => onChange(field.name, nextValue)} required={field.required} />;
