...
function HelperField({ field, value, onChange }: { field: ParentsChildField; value: string; onChange: (name: string, value: string) => void }) {
  const id = `parents-child-${field.name}`; if (isStructuredPartyField(field.name)) return <StructuredPartyField id={id} label={field.label} fieldName={field.name} value={value} onChange={(nextValue) => onChange(field.name, nextValue)} required={field.required} />;
