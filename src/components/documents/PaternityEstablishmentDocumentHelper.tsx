...
import { createPaternityEstablishmentDocxBlob, getPaternityEstablishmentDocxFilename } from "@/lib/paternity-establishment-docx";
import { createPaternityEstablishmentPdfBlob, getPaternityEstablishmentPdfFilename } from "@/lib/paternity-establishment-pdf";
import { getVisiblePaternityEstablishmentFields, validatePaternityEstablishment } from "@/lib/paternity-establishment-validator";
import type { PaternityEstablishmentDecision, PaternityEstablishmentValues } from "@/lib/paternity-establishment-validator";

export function PaternityEstablishmentDocumentHelper({ scenarioKey }: { scenarioKey: PaternityEstablishmentScenarioKey }) {
  const scenario = PATERNITY_ESTABLISHMENT_SCENARIOS[scenarioKey];
  const [values, setValues] = useState<PaternityEstablishmentValues>({});
  const [step, setStep] = useState(0);
  const [decision, setDecision] = useState<PaternityEstablishmentDecision | null>(null);
  const [draft, setDraft] = useState("");
  const [message, setMessage] = useState("");
  const fields = useMemo(() => getVisiblePaternityEstablishmentFields(scenarioKey, values), [scenarioKey, values]);
  ...
  const rules = getPaternityEstablishmentRules(scenarioKey, decision?.legalPath);

  function setField(name: string, value: string) { setValues((current) => ({ ...current, [name]: value })); setDecision(null); setDraft(""); setMessage(""); }
  function advance(event: FormEvent) { event.preventDefault(); if (!field || !canAdvance) return; if (safeStep < fields.length - 1) return setStep(safeStep + 1); const result = validatePaternityEstablishment(scenarioKey, values); setDecision(result); setDraft(""); }
  function goBack() { if (decision) return setDecision(null); setStep((current) => Math.max(0, current - 1)); }
  async function downloadPdf() { if (!decision) return; try { downloadBlob(await createPaternityEstablishmentPdfBlob({ ...decision, draftText: draft }), getPaternityEstablishmentPdfFilename(decision.outcomeKey)); setMessage("PDF сформирован. Проверьте статус готовности на первой странице."); } catch { setMessage("PDF не сформирован. Попробуйте ещё раз."); } }
  async function downloadDocx() { if (!draft) return; try { downloadBlob(await createPaternityEstablishmentDocxBlob(draft), getPaternityEstablishmentDocxFilename(scenario.documentSlug)); setMessage("DOCX сформирован с обязательной маркировкой судебного черновика."); } catch { setMessage("DOCX не сформирован. Используйте PDF или измените ответы."); } }

  return <section id="fill-online" className="scroll-mt-24 border border-line bg-white p-5 shadow-sm sm:p-6">
    <p className="text-sm font-semibold uppercase tracking-wide text-trust">Проверка ситуации</p><h2 className="mt-2 text-2xl font-semibold text-ink">{scenario.mainDocument}</h2>
    <p className="mt-3 max-w-3xl text-sm leading-6 text-zinc-700">Сервис проверяет запись об отце, возраст, основание обращения и безопасно разделяет ЗАГС и суд. Он не устанавливает происхождение ребёнка и не заменяет официальный бланк.</p>
...
function HelperField({ field, value, onChange }: { field: PaternityEstablishmentField; value: string; onChange: (name: string, value: string) => void }) {
  const id = `paternity-establishment-${field.name}`; if (isStructuredPartyField(field.name)) return <StructuredPartyField id={id} label={field.label} fieldName={field.name} value={value} onChange={(nextValue) => onChange(field.name, nextValue)} required={field.required} />;
  if (field.type === "searchable") return <div className="grid gap-2 text-sm font-semibold text-ink">{field.name === "courtSource" || field.name === "courtWebsite" ? <OfficialCourtHelp /> : null}<SearchableSelect id={id} label={field.label} value={value} onChange={(next) => onChange(field.name, next)} options={(field.options ?? []).map((option) => ({ id: option.value, label: option.label }))} required={field.required} />{field.hint ? <span className="text-xs font-normal leading-5 text-zinc-600">{field.hint}</span> : null}</div>;
