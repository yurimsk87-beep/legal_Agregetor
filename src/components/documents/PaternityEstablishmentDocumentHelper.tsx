...
export function PaternityEstablishmentDocumentHelper({ scenarioKey }: { scenarioKey: PaternityEstablishmentScenarioKey }) {
  ...
  function advance(event: FormEvent) { event.preventDefault(); if (!field || !canAdvance) return; if (safeStep < fields.length - 1) return setStep(safeStep + 1); const result = validatePaternityEstablishment(scenarioKey, values); setDecision(result); setDraft(result.draftText); }
...
function HelperField({ field, value, onChange }: { field: PaternityEstablishmentField; value: string; onChange: (name: string, value: string) => void }) {
  const id = `paternity-establishment-${field.name}`;
