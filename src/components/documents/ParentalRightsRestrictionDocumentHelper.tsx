...
function HelperField({ field, value, onChange }: { field: ParentalRightsRestrictionField; value: string; onChange: (name: string, value: string) => void }) {
  const id = `parental-rights-restriction-${field.name}`; if (isStructuredPartyField(field.name)) return <StructuredPartyField id={id} label={field.label} fieldName={field.name} value={value} onChange={(nextValue) => onChange(field.name, nextValue)} required={field.required} />;
