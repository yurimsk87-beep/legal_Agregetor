...
import { PARENTAL_RIGHTS_DEPRIVATION_SCENARIOS, type ParentalRightsDeprivationField, type ParentalRightsDeprivationScenarioKey } from "@/data/parental-rights-deprivation-route";
import { createParentalRightsDeprivationDocxBlob, getParentalRightsDeprivationDocxFilename } from "@/lib/parental-rights-deprivation-docx";
import { createParentalRightsDeprivationPdfBlob, getParentalRightsDeprivationPdfFilename } from "@/lib/parental-rights-deprivation-pdf";
...
import type { ParentalRightsDeprivationDecision, ParentalRightsDeprivationValues } from "@/lib/parental-rights-deprivation-validator";

export function ParentalRightsDeprivationDocumentHelper({ scenarioKey }: { scenarioKey: ParentalRightsDeprivationScenarioKey }) {
  const scenario = PARENTAL_RIGHTS_DEPRIVATION_SCENARIOS[scenarioKey];
  const [values, setValues] = useState<ParentalRightsDeprivationValues>({});
  const [step, setStep] = useState(0);
  const [decision, setDecision] = useState<ParentalRightsDeprivationDecision | null>(null);
  const [draft, setDraft] = useState("");
  const [message, setMessage] = useState("");
  const fields = useMemo(() => getVisibleParentalRightsDeprivationFields(scenarioKey, values), [scenarioKey, values]);
  ...
  const rules = getParentalRightsDeprivationRules(scenarioKey, decision?.legalPath);
  ...
  function advance(event: FormEvent) {
    ...
    const result = validateParentalRightsDeprivation(scenarioKey, values);
    setDecision(result);
    setDraft("");
  }
  ...
  async function downloadPdf() {
    if (!decision) return;
    try {
      downloadBlob(await createParentalRightsDeprivationPdfBlob({ ...decision, draftText: draft }), getParentalRightsDeprivationPdfFilename(decision.outcomeKey));
  ...
  async function downloadDocx() {
    if (!draft) return;
    try {
      downloadBlob(await createParentalRightsDeprivationDocxBlob(draft), getParentalRightsDeprivationDocxFilename(scenario.documentSlug));
  ...
  return (
    <section id="fill-online" className="scroll-mt-24 border border-line bg-white p-5 shadow-sm sm:p-6">
      ...
      <p className="mt-3 max-w-3xl text-sm leading-6 text-zinc-700">Сервис проверяет закрытый перечень оснований и безопасные ограничения. Он не устанавливает виновность родителя и не прогнозирует решение суда.</p>
...
function HelperField({ field, value, onChange }: { field: ParentalRightsDeprivationField; value: string; onChange: (name: string, value: string) => void }) {
  const id = `parental-rights-deprivation-${field.name}`; if (isStructuredPartyField(field.name)) return <StructuredPartyField id={id} label={field.label} fieldName={field.name} value={value} onChange={(nextValue) => onChange(field.name, nextValue)} required={field.required} />;
