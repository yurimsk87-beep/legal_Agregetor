...
import type { SurrogacyOriginDecision } from "@/lib/surrogacy-origin-validator";

export function buildSurrogacyOriginPdfText(result: SurrogacyOriginDecision) {
...
export async function createSurrogacyOriginPdfBlob(result: SurrogacyOriginDecision) {
  ...
  const content: Content[] = [
    { text: result.resultLabel, style: "eyebrow" },
    { text: result.documentTitle, style: "title" },
    { text: "Готово к подаче: нет. Требуется юридическая проверка.", style: "warning" },
    ...result.notices.map((text) => ({ text, margin: [0, 0, 0, 6] }) as Content),
    { text: "Подготовленные сведения", style: "heading" },
    { ul: result.preparedData.map((item) => `${item.label}: ${item.value}`) },
  ...
  const definition: TDocumentDefinitions = {
    ...
    footer: (page, pages) => ({ margin: [42, 10, 42, 0], columns: [
      { text: `Правовая сверка: ${SURROGACY_ORIGIN_REVIEWED_AT}`, style: "footer" },
      { text: `${page}/${pages}`, alignment: "right", style: "footer" }
    ] }),
    content,
    styles: { eyebrow: { fontSize: 10, bold: true, color: "#166534", margin: [0, 0, 0, 8] },
      title: { fontSize: 20, bold: true, margin: [0, 0, 0, 12] },
      warning: { fontSize: 11, bold: true, color: "#92400e", margin: [0, 0, 0, 12] },
