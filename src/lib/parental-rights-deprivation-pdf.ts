...
import type { ParentalRightsDeprivationDecision } from "@/lib/parental-rights-deprivation-validator";

export function getParentalRightsDeprivationPdfFilename(outcomeKey: string) {
  return `${outcomeKey}.pdf`;
}

export function buildParentalRightsDeprivationPdfText(decision: ParentalRightsDeprivationDecision) {
...
export async function createParentalRightsDeprivationPdfBlob(decision: ParentalRightsDeprivationDecision) {
  ...
  const definition: TDocumentDefinitions = {
    ...
    footer: (page, pages) => ({ margin: [42, 10, 42, 0], columns: [{ text: `Правовая сверка: ${PARENTAL_RIGHTS_DEPRIVATION_REVIEWED_AT}`, style: "footer" }, { text: `${page}/${pages}`, alignment: "right", style: "footer" }] }),
    content,
