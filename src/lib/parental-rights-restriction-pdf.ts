...
import type { ParentalRightsRestrictionDecision } from "@/lib/parental-rights-restriction-validator";

export function getParentalRightsRestrictionPdfFilename(outcomeKey: string) {
  return `${outcomeKey}.pdf`;
}

export function buildParentalRightsRestrictionPdfText(decision: ParentalRightsRestrictionDecision) {
  return [decision.resultLabel, decision.documentTitle, "Готово к подаче: нет", ...decision.notices, "", "Подготовленные сведения:", ...decision.preparedData.map((item) => `- ${item.label}: ${item.value}`), "", "Что делать дальше:", ...decision.filingSteps.map((item, index) => `${index + 1}. ${item}`), ...(decision.draftText ? ["", decision.draftText] : [])].join("\n");
}

export async function createParentalRightsRestrictionPdfBlob(decision: ParentalRightsRestrictionDecision) {
  if (!decision.pdfAvailable) throw new Error("PDF недоступен для срочного состояния.");
  ...
  const content: Content[] = [
    ...
    { text: "НЕ ГОТОВ К ПОДАЧЕ. Проверьте статус и дальнейшие шаги.", style: "warning" },
  ...
  const definition: TDocumentDefinitions = {
    ...
    footer: (page, pages) => ({ margin: [42, 10, 42, 0], columns: [{ text: `Правовая сверка: ${PARENTAL_RIGHTS_RESTRICTION_REVIEWED_AT}`, style: "footer" }, { text: `${page}/${pages}`, alignment: "right", style: "footer" }] }),
    content,
