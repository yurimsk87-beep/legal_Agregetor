...
const documentSpecs: DocumentSpec[] = [
  ...
  // Семья
  {
    ...
    templateSlug: "zayavlenie-v-zags",
    shortIntro:
      "Одна страница для выбора нужного заявления в ЗАГС: заключение брака, перемена имени, повторное свидетельство или справка, исправление записи.",
    shortDescription:
      "Выберите цель обращения в ЗАГС — покажем официальную форму, документы, срок, госпошлину и безопасный способ подачи.",
    description:
      "Заявление в орган ЗАГС нельзя сделать одним универсальным текстом для всех случаев. Для заключения брака, перемены имени, повторных документов и исправления записи применяются разные официальные формы. Эта страница помогает выбрать правильный вариант и не подменяет установленный бланк свободным текстом.",
    heroDescription:
      "Выберите вариант заявления в орган ЗАГС. Сервис покажет, какая официальная форма применяется, какие документы подготовить, куда подать и когда вместо внутренней генерации безопаснее использовать официальный электронный сервис или бланк ЗАГС.",
    whenToUse: [
    ...
    whereToSubmit:
      "В орган ЗАГС, через МФЦ или через официальный электронный сервис, если он доступен для выбранной услуги.",
    filingProcedure: [
    ...
    relatedSituations: [{ title: "Брак и ЗАГС", slug: "brak-zags-i-smena-familii" }],
    faq: [
      {
    ...
    disclaimer:
      "Страница не заменяет официальный бланк ЗАГС. Если точную форму нельзя подтвердить для вашей процедуры, используйте официальный сервис или бланк органа ЗАГС."
  },
