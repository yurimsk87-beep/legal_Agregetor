...
function PaternityContestScenarioDetails({ problemPath, scenario }: { problemPath: string; scenario: PaternityContestScenario }) {
  return <section className="mt-7">
    ...
    <Link href={`/documents/${scenario.documentSlug}/#fill-online`} target="_blank" rel="noopener noreferrer" className="mt-6 inline-flex min-h-11 items-center justify-center rounded-md bg-trust px-5 py-3 text-sm font-semibold text-white focus:outline-none focus:ring-2 focus:ring-trust/30">Проверить ситуацию</Link>
...
function ChildSupportScenarioDetails({ problemPath, scenario }: { problemPath: string; scenario: ChildSupportScenario }) {
  return (
    <section className="mt-7">
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2"><InfoBox title="Основной результат" text={scenario.mainDocument} /><InfoBox title="Куда обращаться" text={scenario.filing} /><InfoBox title="Срок" text={scenario.term} /><InfoBox title="Расходы" text={scenario.fee} /></div>
      <div className="mt-5 border-l-4 border-amber-400 bg-amber-50 p-4 text-sm leading-6 text-amber-950">{scenario.warning}</div>
      <Link href={`/documents/${scenario.documentSlug}/#fill-online`} target="_blank" rel="noopener noreferrer" className="mt-6 inline-flex min-h-11 items-center justify-center rounded-md bg-trust px-5 py-3 text-sm font-semibold text-white focus:outline-none focus:ring-2 focus:ring-trust/30">Подготовить документ</Link>
...
function GuardianshipScenarioDetails({ problemPath, scenario }: { problemPath: string; scenario: GuardianshipScenario }) {
  return (
    <section className="mt-7">
      ...
      <Link href={`/documents/${scenario.documentSlug}/#fill-online`} target="_blank" rel="noopener noreferrer" className="mt-6 inline-flex min-h-11 items-center justify-center rounded-md bg-trust px-5 py-3 text-sm font-semibold text-white hover:bg-ink focus:outline-none focus:ring-2 focus:ring-trust/30">Подготовить документ</Link>
...
function DivorceScenarioDetails({ problemPath, scenario }: { problemPath: string; scenario: DivorcePropertyScenario }) {
  return (
    <section className="mt-7">
      ...
      <Link href={`/documents/${scenario.documentSlug}/#fill-online`} target="_blank" rel="noopener noreferrer" className="mt-6 inline-flex min-h-11 items-center justify-center rounded-md bg-trust px-5 py-3 text-sm font-semibold text-white hover:bg-ink focus:outline-none focus:ring-2 focus:ring-trust/30">
...
function ScenarioDetails({ problemPath, scenario }: { problemPath: string; scenario: ZagsScenario }) {
  return (
    <section className="mt-6 rounded-lg border border-line bg-white p-5 shadow-sm sm:p-6">
      ...
      <div className="mt-6">
        <Link href={`/documents/${ZAGS_PROBLEM_ROUTE.documentSlug}/?variant=${scenario.key}#fill-online`} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-11 items-center justify-center rounded-md bg-trust px-5 py-3 text-sm font-semibold text-white hover:bg-ink focus:outline-none focus:ring-2 focus:ring-trust/30">
          Подготовить документ
        </Link>
      </div>
